...
if ~isfield(r.c_obs, 'predorpost')
    r.c_obs.predorpost = 1;
end

switch r.c_obs.predorpost
    case 1
        pop = 1;
    case 2
        pop = 3;
    otherwise 
        error('tapas:hgf:sigma_binary:input', ...
            'Unknown preorpost');
end
...
if size(r.u,2) == 1
    ...
    probc = 1./(1+exp(-decision_noise.*(2.*x-1).*(2.*y-1)));
end
...
if size(r.u,2) == 3
    % Probability of observed choice
    probc = 1./(1+exp(-decision_noise.*(r1.*x-r0.*(1-x)).*(2.*y-1)));
end
